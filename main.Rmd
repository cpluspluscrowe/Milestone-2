...
library(ISLR)
library(caret)
trControl <- trainControl(method = 'cv', number = 3, savePredictions = TRUE, classProbs = TRUE, summaryFunction = twoClassSummary)
tuneGrid <- expand.grid(k = 1:2)
knn_fit <- train(as.formula(model_form), data = data_train, method = model_type, tuneGrid = tuneGrid,trControl = trControl, preProc = c("bagImpute","center", "scale"), na.action = na.pass, metric = 'ROC')
